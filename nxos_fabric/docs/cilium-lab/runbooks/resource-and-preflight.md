...
multisite の実行先は clab02。2026-09-13 時点の起動前資源と構成候補は
[clab02 起動前の構成・イメージ検討](../design/multisite-clab02-startup-options.md) を参照する。
下記の過去の測定値は、その測定時点の構成に対する結果である。
...
| Node Fabric MTU | `9150` | Cilium 基準 MTU `9050`／Pod 経路 MTU `9000` を収容する。詳細は [MTU 設計](../architecture.md) |
| Kind Node image | digest 固定の `v1.35.5` | tag だけでなく digest を比較する |

必要な Fabric／DCI port は [Cluster Mesh Fabric／DCI 境界設計](../design/clustermesh-fabric-dci-and-acceptance.md)で
管理する。
