...
[リソース設計と preflight](../runbooks/resource-and-preflight.md)の合否基準で測定する。
...
- 後からの変更: Helm upgrade と Agent／Operator rollout が必要。Egress IP は選定 Node の Fabric
  ...
  有効化しない。Stage 5 合格後に
  [Egress Gateway／Cluster Mesh 同時有効化試験](../tests/egress-clustermesh-coexistence-test.md)へ従い、
...
- 詳細な退避手順、NX-OS 側手順、Test ID:
  [Cilium BGP 経路退避とメンテナンス設計](../runbooks/bgp-maintenance-and-route-drain.md)
...
設計根拠の公式 URL と確認日は [参照 URL 台帳](../reference/references.md)で管理する。
