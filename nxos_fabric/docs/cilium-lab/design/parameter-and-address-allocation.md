...
| Cilium 基準 MTU／Pod interface MTU | `9050` | `Assigned` |
| Pod の経路 MTU | `9000` | IPv4 VXLAN・暗号化なし |
| kind Node Fabric interface MTU | `9150` | `Assigned` |
...
Node の `eth1`、`eth2`、`bond0`、`bond0.<local-VLAN>` は `9150` とする。Cilium Helm values では
`MTU: 9050` を明示し、IPv4 VXLAN 分 `50` byte を引いた Pod の経路 MTU を `9000` とする。
Node が IP 全長 `9150` byte を送る場合、Leaf の IPv4 VXLAN を含む Fabric path には IP MTU `9200` が必要になる。
暗号化・IPv6 underlay・WAN の条件は [architecture の MTU 設計](../architecture.md#mtu-9100-plan) を参照する。
...
| Address owner | k02 は初期化 DaemonSet が Policy 適用前に Node へ設定する。Cilium による動的割り当ては行わない | `Assigned` |
...
Egress IP は k02 の [初期化 manifest](../../../nxos_singlesite/k8s_kind/k02/cilium/manifests/egress-interface-init/README.md) で
`egress0` に設定する。Cilium は IP の動的割当を行わず、
...
5. Node Fabric MTU `9150` と Fabric underlay MTU `9214`／`9216` の end-to-end path を確認する。
