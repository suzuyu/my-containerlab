...
いずれかの CLI が表示されない場合は、[クライアントツール準備手順](../runbooks/client-tools.md)を先に実行する。
...
| `clustermesh.config.clusters[].ips` | 未設定 | 各 API の固定 IPv4／IPv6 VIP | Mesh 内部の host alias で FQDN を解決する | VIP／SAN／remote config を同時変更 |
...
| API `updateStrategy.rollingUpdate` | `maxSurge: 1`、`maxUnavailable: 0` | `maxSurge: 0`、`maxUnavailable: 1` | 2 worker の required anti-affinity で更新が停止しないよう、1 Pod ずつ置き換える | replica／配置制約と合わせて見直す |
...
保存 values では `clustermesh.config.clusters[].ips` に上表の IPv4／IPv6 VIP を指定し、
Mesh 内部の host alias で FQDN を解決する。API Pod の `hostAliases` と remote config の
`cilium-host-aliases` は chart が生成する。通常の Pod／ホストの DNS に A／AAAA を登録する方式ではない。
外部 DNS を使う場合は `ips` の代わりに `address` を設定し、両 site での A／AAAA 解決を別途確認する。
Cluster Mesh は mTLS を使用するため、全 cluster が共通 root CA を信頼する。
Git には CA private key、certificate Secret、remote config、kubeconfig を保存しない。
...
詳細は [BGP maintenance と route drain](../runbooks/bgp-maintenance-and-route-drain.md)を参照する。
...
```bash
for context in kind-adc-k02 kind-bdc-k03; do
  mesh_pod="$(kubectl --context "$context" -n kube-system get pod \
    -l k8s-app=clustermesh-apiserver -o jsonpath='{.items[0].metadata.name}')"
  kubectl --context "$context" -n kube-system exec "$mesh_pod" -c kvstoremesh -- \
    /usr/bin/clustermesh-apiserver kvstoremesh-dbg troubleshoot
done
...
| `CM-02` | 名前解決／API VIP | 両 site の Mesh 内部で FQDN が固定 IPv4／IPv6 VIP に解決される。外部 DNS 方式なら A／AAAA も確認する |
