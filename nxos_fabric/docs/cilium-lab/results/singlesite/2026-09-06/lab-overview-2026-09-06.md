> 整理前の README に記載されていた 09-06 時点の概要・初期方針。後日の変更を反映した現在値ではない。現在の構成は [architecture](../../../architecture.md)、進捗は [status](../../../status.md) を参照する。
...
1. [要件・設計台帳](../../../design/requirements-and-design.md)
2. [Cilium 設定パラメータ設計](../../../design/cilium-configuration-parameters.md)
3. [パラメータ・アドレス割り当て台帳](../../../design/parameter-and-address-allocation.md)
4. [構成設計と構成図](../../../architecture.md)
5. [現環境から初期構築までの変更計画](../../../design/current-environment-change-plan.md)
6. [リソース設計と preflight](../../../runbooks/resource-and-preflight.md)
7. [BGP 終端設計と設定変更案](../../../design/bgp-config-change-proposal.md)
8. [Cilium Service VIP 経路集約の比較設計](../../../design/bgp-route-aggregation-design.md)
9. [Cilium BGP 経路退避とメンテナンス設計](../../../runbooks/bgp-maintenance-and-route-drain.md)
10. [Cluster Mesh Fabric／DCI 境界設計](../../../design/clustermesh-fabric-dci-and-acceptance.md)
11. [検証ワークロード設計](../../../tests/test-workloads.md)
12. [Network Policy／Tetragon 検証計画](../../../tests/network-policy-and-tetragon-test-plan.md)
13. [Egress Gateway／Cluster Mesh 同時有効化試験](../../../tests/egress-clustermesh-coexistence-test.md)
14. [Fabric 側 Kubernetes client と CLI 準備](../../../runbooks/client-tools.md)
15. [段階的な構築計画](../../../build-plan.md)
16. [Stage 成果物台帳](../../../reference/stage-artifact-inventory.md)
17. [2026-09-06 single-site k02 検証ステータス](validation-status-2026-09-06.md)（[2026-08-30 の履歴](../2026-08-30/validation-status-2026-08-30.md)）
18. [試験課題台帳](../../../test-issue-register.md)
19. [参照 URL 台帳](../../../reference/references.md)

Egress Gateway の実施時は [構築・試験手順](../../../tests/egress-gateway-test-plan.md) を使用する。
今回の接続先・配置パス・転送運用は [実際の試験環境：single-site k02](../../../runbooks/execution-environment-singlesite-k02.md) に分離し、
別環境へ流用するときは環境メモの設定値を置き換える。

connectivity 試験の検証用 CLI を再作成する場合は、
[公式ソース取得・修正パッチ適用・ビルド手順](../../../reference/cli-lab-flowfix-build.md) を使用する。
...
- 最新の適用範囲・判定・残課題は [2026-09-06 検証ステータス](validation-status-2026-09-06.md) を正本とする。
  [2026-08-30 のスナップショット](../2026-08-30/validation-status-2026-08-30.md) は当時の履歴として保持する
...
- 確定した設計値と segment 内の host assignment は
  [パラメータ・アドレス割り当て台帳](../../../design/parameter-and-address-allocation.md)で管理する
...
- `stable` URLだけに依存せず、採用バージョンと確認日を[参照URL台帳](../../../reference/references.md)へ残す。
...
[Cilium 設定パラメータ設計](../../../design/cilium-configuration-parameters.md)の一覧表と項目別の節を同時に更新する。
項目別の節には、項目の意味、選択肢、採用 Cilium version の既定値、本ラボでの初期設定、選定理由、後からの変更可否、
変更方法、変更時の影響と確認事項を記録する。

Egress のアドレス割当と個別経路は [Egress 専用 IP・BGP 経路設計](../../../design/egress-gateway-routed-design.md) を参照する。
