...
この文書は [2026-08-30 のスナップショット](../2026-08-30/validation-status-2026-08-30.md) を引き継ぎ、
...
接続先・配置パス・実施セッションの詳細は [実際の試験環境](../../../runbooks/execution-environment-singlesite-k02.md)、
...
[パッチ・由来・再ビルド手順](../../../reference/cli-lab-flowfix-build.md) に判定変更の範囲を記載した。
...
詳細は [Network Policy／Tetragon 手順](../../../tests/network-policy-and-tetragon-test-plan.md) と Git 管理外の各 `tg*-result.md` に分離した。
...
再実行用の構成図・端末 A／B のコマンド・出力例は [Egress Gateway 試験手順](../../../tests/egress-gateway-test-plan.md) に記載する。
...
意味・採用条件・維持／解除方法は [checksum 運用手順](../../../runbooks/checksum-compat-operations.md)、
候補の評価は [kernel 対応方針](../../../runbooks/kernel-compatibility-policy.md) と [候補評価](../../../reference/kernel-checksum-candidates.md) を参照する。
