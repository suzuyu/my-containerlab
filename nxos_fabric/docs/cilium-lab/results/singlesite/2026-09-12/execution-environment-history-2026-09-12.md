> 2026-09-06〜13 の作業履歴。整理前の環境メモを保持する。現在の接続・準備は [実行環境](../../../runbooks/execution-environment-singlesite-k02.md)、現在の進捗は [status](../../../status.md) を参照する。
...
更新ファイルの配置、事前確認、Egress Gateway の基本試験と撤去を実施した。試験の目的・操作・判定は [Egress Gateway 構築・試験手順](../../../tests/egress-gateway-test-plan.md) を参照する。
...
- `nxos_fabric/docs/cilium-lab/tests/egress-gateway-test-plan.md`
...
次は [kernel 判定と対応方針](../../../runbooks/kernel-compatibility-policy.md) に従い、互換設定の維持・解除条件を整えてから、Egress の残りの受入を進める。
...
ユーザー `suzuyu` の linger は no → yes に変更した。停止・解除手順は [運用手順](../../../runbooks/checksum-compat-operations.md) を参照する。
...
最新の恒久候補の評価は [kernel 候補調査](../../../reference/kernel-checksum-candidates.md) にまとめる。
...
証跡は `raw/egress-invalid-ByUgGZaH`。実施構成の図と端末 A/B 手順は [12.1～12.4 節](../../../tests/egress-gateway-failure-tests.md#121-異常系-2-ケースの共通準備) にある。
...
## 19. 初期化 DaemonSet の統合と検証（2026-09-12）

[統合試験結果](egress-interface-init-validation-2026-09-12.md) が初期化方式の最新状態。
セッションは `egress-init-ypOuk9eg`。clab01 の k02 は稼働しており、再起動せずに適用・検証した。
ローカル Docker で Node が見つからなかったことを、実行ホストの停止とした以前の説明は訂正する。

Egress IP は `nodes.json` と独立 DaemonSet で管理する。通常の試験後も初期化 DaemonSet・ConfigMap・label・
`egress0` は維持し、試験用 Policy・BGP 広報・namespace・サーバだけを撤去する。
基本 HTTP 48/48、新規 Pod 240/240、変更前後の LB 各 12/12 が成功した。
Node 再起動後の初期化は未検証。過去の節にある手動 IP 作成・全 IP 撤去は当時の試験履歴である。

## 20. single-site 設定照合・全体回帰・性能切り分け（2026-09-12〜13）

[試験結果](singlesite-regression-validation-2026-09-12.md) が今回の再開位置と最新状態。
セッション `singlesite-regression-a34lz3g0` は clab01 の k02 を使用し、証跡の日付 directory は `2026-09-12`、
実際の取得時刻は UTC／JST で保持する。kernel 更新は multisite 試験後、新規 Node への初期化適用は multisite 構築時に確認する。

Leaf 4 台・Node 3 台の MTU／LACP、Fabric IP、Helm の明示 63 項目、初期化設定を照合した。
全体回帰は 80/82 tests 成功、2 tests／3 actions 失敗、50 skip。旧 IP の hostNetwork 試験用 Pod 1 個を
再作成した限定再試験は 1 test／6 actions 成功した。累積 drop counter の検出は今回の増加と分けて記録する。

性能切り分けでは専用 UDP 診断コードと TCP socket 情報を記録する専用 probe を使用する。
標準 CLI／常用 probe は置換せず、試験で使用したソース・バイナリのハッシュを証跡へ保存する。
一時 Policy・BGP 広報・試験 namespace／サーバを撤去し、初期化 DaemonSet・ConfigMap・label・`egress0` は維持する。

主測定は UDP 40 条件＋capture 停止対照 2 条件、TCP 12 条件。TCP は全量回収したが再送は残った。
LB 232/232 と最終照合は成功。Fabric の遅延・順序逆転・欠落区間と socket 側未回収は TI-004 で継続する。
